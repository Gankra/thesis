...
At a base level, Rust manages data ownership with an \emph{affine type system} \cite{tov2011practical}. The
literature often describes affine types as being usable \emph{at most once}, but from
Rust's perspective affine typing primarily means values are \emph{uniquely owned}.
...
important. Such types can opt into \emph{copy} semantics. Copy types behave like any
...
issue. One solution is to simply \emph{copy} the data to all the consumers. Each
...
the language: regions \cite{swamy2006safe}.
...
Yet another way is to treat all pointers into the array as pointers \emph{to}
the array, and simply forbidding mutation \emph{through} pointers. All mutating operations
...
{rust}{&mut} and \mintinline{rust}{&} respectively. Shared references are exactly as we described: they can be
...
acquiring its lock. This is in turn made sound by affinity and the borrow checker ensuring the
...
isn't actually true. In fact, basically \emph{every} language has unsafe bits that
